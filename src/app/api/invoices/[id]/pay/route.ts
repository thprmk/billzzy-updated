...
import { generateBillNumber } from '@/lib/generateBillNumber'; // Changed to named import

export async function PATCH(
  req: Request,
  { params }: { params: { id: string } } // This is the modern, correct signature
) {
  ...
  const invoiceId = Number(params.id); // This is the modern, correct way to get the ID
  ...
  try {
    const updatedInvoice = await prisma.$transaction(async (tx) => {
      ...
      const newBillNo = await generateBillNumber(tx, organisationId, 'online');
