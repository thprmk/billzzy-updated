    ...
    try {
      ...
      const bills = await prisma.transactionRecord.findMany({
        ...
        include: {
          ...
          // Include TransactionShipping to get shipping details
      ...
      const formattedBills: BillResponse[] = bills.map((bill) => {
        ...
        // Extract shipping data (if any)
        // Assuming a one-to-one relationship, TransactionShipping is an array of length 0 or 1
        const shippingData = bill.TransactionShipping?.[0] || null;
        ...
        return {
          ...
          notes: bill.notes,
          // Include the shipping info if present
          shipping: shippingData ? {
            methodName: shippingData.methodName,
            methodType: shippingData.methodType,
            totalCost: shippingData.totalCost,
            baseRate: shippingData.baseRate,
            weightCharge: shippingData.weightCharge,
